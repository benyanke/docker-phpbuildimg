...
RUN add-apt-repository -y ppa:ondrej/php && export DEBIAN_FRONTEND=noninteractive && apt-get update  && apt-get install -y jq python yamllint python-pip unzip libxml2-dev libldb-dev php7.0-ldap php7.0-json php7.0-mcrypt php7.0-curl php7.0-gd --no-install-recommends && apt-get clean && rm -rf /var/lib/apt/lists/*
